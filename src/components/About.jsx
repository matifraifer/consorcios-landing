      ...
      <Container maxWidth="lg">
        <Grid container spacing={{ xs: 6, md: 10 }} sx={{ alignItems: 'center' }}>
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            <Typography sx={{ fontSize: '0.95rem', color: colors.textMuted, lineHeight: 1.8, mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {STATS.map(({ value, label }) => (
                <Box key={label} sx={{
                  ...
                  bgcolor: colors.bg, borderRadius: radius.md,
                  border: `1px solid ${colors.border}`, p: 3,
                }}>
                  <Typography sx={{
                    fontSize: '2rem', fontWeight: 800, color: colors.accent,
                    lineHeight: 1, minWidth: 68, letterSpacing: '-0.03em',
                  }}>
                    {value}
                  </Typography>
                  <Typography sx={{ fontSize: '0.9rem', color: colors.text, fontWeight: 500 }}>
                    {label}
                  </Typography>
                </Box>
              ))}
